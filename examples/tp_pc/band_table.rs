...
/// `true` keeps raw ticks. `decimals` sets the fractional digits
/// on every time cell (counts stay integer), like iiac-perf's
/// `--decimals`.
pub fn render(kind: &str, name: &str, hist: &Histogram<u64>, as_ticks: bool, decimals: usize) {
